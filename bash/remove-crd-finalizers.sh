...
resources=$(kubectl api-resources --namespaced=true --verbs=list | awk 'NR>1 {print $1}' | grep -Ev '^(events|limitranges|challenges|controllerrevisions|ephemeralreports|policyreports)$')
...
for resource in $resources; do
  ...
  for instance in $instances; do
    ...
    if ! kubectl patch "$instance" -n "$NAMESPACE" --type=merge -p "$PATCH_PAYLOAD" >/dev/null; then
